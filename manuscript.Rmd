...
representation for these data is one which can hold an unlimited number of motif
...
data.frame is just a data.frame, it can be manipulated using base R or
`tidyverse` functions, just like regular R data.frames, lowering the cognitive
overhead to interfacing with these structures.
...
any motif list. Using `universalmotif` functions to import motif data,
the `memes` function `as_universalmotif_dataframe()` converts any `universalmotif` object
into a `universalmotif` data.frame.  Manipulation of linked metadata columns
will result in updating the linked `universalmotif` object when calling
