...
`tidyverse` functions, making it easier to interface with these
structures.
...
non-meme file formats \cite{motifdb, universalmotif}. Once imported in `universalmotif` format
...
discarding columns that cannot be stored within `universalmotif` format.
Therefore, this data structure empowers users to import and analyze motif lists
from a diverse set of sources for use with `memes` or for simple manipulation of
...
representative plot of these data is shown in \ref{fig:tomtom_example}, while
the full results can be viewed in \ref{tab:tomtom_table}. This analysis revealed
...
factors br, l(3)neo38, gl, and lola \ref{tab:tomtom_table}. These data support
...
potential factors that also bind with E93 affect chromatin accessibility.
