...
sequences. **TODO: R/Bioconductor transition sentence**. Although tools that
implement these features exist in Bioconductor, they often rely on
proprietary data types or have poor interoperability with other tools, creating
barriers to comprehensive motif analysis within R. The recent addition of
`universalmotif` to the Bioconductor landscape partially addresss these issues
by standardizing motif formats and providing a powerful set of tools for
examining the properties of motifs. However the Bioconductor motif analysis
landscape remains incomplete \cite{universalmotif}.
...
these factors act as barriers to adoption, preclude deeper analysis of the
data, and limit communication of results to the scientific community. 
...
expressed factors. By maintaining compatability with `universalmotif`, `memes`
outputs can be readily used in a diverse set of tools beyond the `memes`
framework.
...
The `universalmotif` data structure is a powerful R/Bioconductor representation
for motif matrices and their associated metadata \cite{universalmotif}. However,
`universalmotif` objects are not ideal for performing bulk manipulation or
queries of metadata, nor do they support complex data structures such as nested
elements. R data.frames support nesting arbitrary data types within columns, but
are not sufficient to properly store motif matrices. In order to create an
extensible, nested data structure for tracking MEME Suite data, `memes`
leverages the power of R data.frames and the `universalmotif` format by nesting
`universalmotif` objects within data.frames. These universalmotif data.frames
store information about motifs along rows, where each column stores metadata
associated with a motif, including a `motif` column holding a `universalmotif`
representation of that motif, which can be manipulated by altering columns of
the data.frame (described in detail below). Adding one-to-many relationships within this
structure is trivial, as additional unviersalmotif data.frames can be nested
within eachother. These universalmotif data.frames form the basis for a majority
of `memes` data outputs. These structures are also valid input types to `memes`
functions, and when used as such, output data are appended as new columns to the
input data, ensuring data provenance. Finally, because the universalmotif
data.frame is a standard R data.frame, it can be manipulated using base R or
`tidyverse` functions, lowering the cognitive overhead to interfacing with these
structures.
...
any motif list. Using `universalmotif` functions to import motif data, the
`memes` function `as_universalmotif_dataframe()` converts any `universalmotif`
object into a `universalmotif` data.frame.  Manipulation of linked metadata
columns will result in updating the linked `universalmotif` object when calling
`update_motifs()` to retain the data.frame structure, or when calling
`as_universalmotif()` to convert the data.frame back into `universalmotif`
format. In this way, `memes` enables large-scale motif database manipulation &
analysis within the preexisting R framework.

The universalmotif data.frames generated by memes are not limited to storing
motifs from MEME Suite data. They can also be used to manipulate or store other
descriptive properties of motifs from multiple sources. `universalmotif`
provides several import functions which can convert motifs from external
databases queried from the `MotifDb` package, or from files in a variety of
non-meme file formats \cite{motifdb}. Once imported in `universalmotif` format
the `memes` function `as_universalmotif_dataframe()` converts any
`universalmotif` object into a universalmotif data.frame. Similar to MEME Suite
data stored in this format, if changes are made to the linked metadata columns,
the linked universalmotif object can be updated by calling `update_motifs()` to
retain the data.frame structure for further analysis, or by calling
`as_universalmotif()` to convert the data.frame back into universalmotif format,
discarding columns which cannot be stored within `universalmotif` format.
Therefore, this data structure empowers users to import and analyze motif lists
from a diverse set of sources for use with `memes` or simply for manipulating
motif metadata using common R tools. In this way, memes enables large-scale
motif database manipulation & analysis within the preexisting R framework.
...
regions through integration with orthogonal datasets (e.g.  categorizing
transcription factor binding sites by the presence or absence of another
factor). In these instances, motif analysis acts as a powerful tool to examine whether
differences in motif content is associated with membership of distinct
categories (e.g. motifs enriched at sites where both factors are bound vs singly
bound). Although the MEME Suite allows differential enrichment testing, it does
