...
statistical information about their enrichment \cite{meme, dreme}. In this instance, a position
...
with other motif matrices (again with their own metadata) \cite{tomtom}. Thus an ideal
...
presence or absence of another factor \cite{???}. Although the MEME Suite allows
...
fly_factor <- universalmotif::read_meme("data/flyFactorSurveyCleaned.meme") %>% 
  {
    # TODO: why aren't names added by default on import anymore?
    names(.) <- purrr::map_chr(., ~{.x['name']})
    .
  } %>% 
  to_df() %>% 
  # TODO: print method doesn't play nice w/ Rmd
  data.frame
```
